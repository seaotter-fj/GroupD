const config = {
  ...
  secondPageName: document.createElement("h2"), // いらない // こちらも消しちゃいます〜
  inputNum: document.getElementById("inputted-number").value,
...
<div>
  ...
  <div class="d-flex justify-content-center">
    <button class="btn btn-light btn-outline-dark" id="game-start">
      Game Start
    </button>
  </div>
</div>
...
config.startBtn.addEventListener("click", () => {
  ...
  if (
    document.getElementById("inputted-number").value === "" || // 重複してます
    playerName === ""
  ) {
    ...
    config.secondPageName.innerHTML = playerName;
    config.secondPage.append(config.secondPageName);
    let state = setAry(config.inputNum);
    console.log("state:", state);
    setBoxes(config.inputNum);
  }
});
